...
class PanelController extends Controller
{
    ...
    {   
        ...
        if(($wallet->Money+100000)-$wallet->Buy>=$req->Price)
        {
            ...
            $user->Wallet=$this->MyWallet($user->Id)->getData()->wallet??'-';
            return response()->json(['success'=>1,'msg'=>"چالش خریداری شد"]);
        }
    ...
    {   
        $newchall=$this->getData('select',['uid'=>$uId],'newChalls',1);
    ...
    public function getData($type,$param,$function,$sName=null)
    {
        ...
        switch ($function) {
            case 'index':
                $select="select uc.Id,uc.Done,uc.Expired,uc.ExpiredAt,uc.UserId,uc.Answer,uc.Date,uc.Pay,isnull(uc.Price,0) as Money,c.Price,c.Expire,c.[File],c.Options,c.Type,c.Title,c.Body,c.Level, (select cc.Closed from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as Closed , (select cc.Resiver from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as ChatResiver,(select cc.Id from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as ChatId,(select mcc.Body from InterviewChallMsgTbl as mcc inner join InterviewChallChatTbl as cc1  on cc1.Id=mcc.ChatId and cc1.Active=1 and cc1.ChallUserId=uc.Id and  mcc.Active=1 where uc.Answer=mcc.Id  ) as MyAnswer From InterviewChallUserTbl as uc inner join InterviewChallTbl as c on uc.ChallId=c.Id and c.Active=1 and uc.Active=1 and  uc.UserId=".$param['uid']." where ((cast(uc.ExpiredAt as Date) >= cast(GETDATE() as Date) and  cast(uc.Date as Date)  <=  cast(GETDATE() as Date)) or cast(uc.ExpiredAt as Date) =  cast(GETDATE() as Date) ) order By Date,c.Id,uc.ExpiredAt ";
                $update="update  InterviewChallUserTbl set Expired=1 where UserId=".$param['uid']." and Expired <> 1 and ExpiredAt <=GETDATE()";
                break;
            case 'history':
                $select="select uc.Id,uc.Done,uc.Expired,uc.ExpiredAt,uc.UserId,uc.Answer,uc.Date,uc.Pay,isnull(uc.Price,0) as Money,c.Price,c.Expire,c.[File],c.Options,c.Type,c.Title,c.Body,c.Level, (select cc.Closed from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as Closed ,(select cc.Resiver from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as ChatResiver,(select cc.Id from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as ChatId,(select mcc.Body from InterviewChallMsgTbl as mcc inner join InterviewChallChatTbl as cc1  on cc1.Id=mcc.ChatId and cc1.Active=1 and cc1.ChallUserId=uc.Id and  mcc.Active=1 where uc.Answer=mcc.Id  ) as MyAnswer From InterviewChallUserTbl as uc inner join InterviewChallTbl as c on uc.ChallId=c.Id and c.Active=1 and uc.Active=1 and  uc.UserId=".$param['uid']." where (Expired=1 or Done=1) order By Date,c.Id";
                $update="update  InterviewChallUserTbl set Expired=1 where UserId=".$param['uid']." and Expired <> 1 and ExpiredAt <=GETDATE()";        
                break;
            case 'details':
                $select="select top 1 uc.Id,uc.Done,uc.Expired,uc.ExpiredAt,uc.UserId,uc.Answer,uc.Date,uc.Pay,isnull(uc.Price,0) as Money,c.Price,c.Expire,c.[File],c.Options,c.Type,c.Title,c.Body,c.Level, (select cc.Closed from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as Closed ,(select cc.Resiver from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as ChatResiver,(select cc.Id from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as ChatId,(select mcc.Body from InterviewChallMsgTbl as mcc inner join InterviewChallChatTbl as cc1  on cc1.Id=mcc.ChatId and cc1.Active=1 and cc1.ChallUserId=uc.Id and  mcc.Active=1 where uc.Answer=mcc.Id  ) as MyAnswer From InterviewChallUserTbl as uc inner join InterviewChallTbl as c on uc.ChallId=c.Id and c.Active=1 and uc.Active=1 and  uc.Id=".$param['cid'];
            ...
            case 'wallet':
                ...
                break;
            case 'buychall':
                $select="select top 1 uc.Id,uc.Done,uc.Expired,uc.ExpiredAt,uc.UserId,uc.Answer,uc.Date,uc.Pay,isnull(uc.Price,0) as Money,c.Price,c.Expire,c.[File],c.Options,c.Type,c.Title,c.Body, (select cc.Closed from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as Closed ,(select cc.Resiver from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as ChatResiver,(select cc.Id from InterviewChallChatTbl as cc where cc.ChallUserId=uc.Id and cc.Active=1) as ChatId,(select mcc.Body from InterviewChallMsgTbl as mcc inner join InterviewChallChatTbl as cc1  on cc1.Id=mcc.ChatId and cc1.Active=1 and cc1.ChallUserId=uc.Id and  mcc.Active=1 where uc.Answer=mcc.Id  ) as MyAnswer From InterviewChallUserTbl as uc inner join InterviewChallTbl as c on uc.ChallId=c.Id and c.Active=1 and uc.Active=1 and  uc.Id=".$param['chall'];
                $update="update  InterviewChallUserTbl set Pay=".$param['Price'].",PayDate=GETDATE(),ExpiredAt='".$param['expiredAt']."',Expired=0 where Id=".$param['chall']." ";
                break;
            case 'newChalls':
                $select="SELECT c.Id,c.Level,c.Expire FROM InterviewChallTbl as c
                where Level<=
                (select top 1 c2.Level from InterviewChallUserTbl as uc join InterviewChallTbl as c2 on c2.Id=uc.ChallId where uc.UserId=".$param['uid']." and uc.Active=1 and cast(uc.Date as Date)=cast(GETDATE() as Date))
